...
and some catalogs simply provide original image files but no COGs, which may slow down the processing. Furthermore the `rstac` package at the moment cannot work with static STAC catalogs (without API service). For a good reason, STAC API endpoints contain request limits on the number of returned features. When requesting longer time series or larger areas, these limits are quickly reached and it is up to the user to subdivide queries.
